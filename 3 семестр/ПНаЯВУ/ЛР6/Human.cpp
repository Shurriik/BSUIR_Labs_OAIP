...
std::istream& operator>>(std::istream& is, Human& human)
{
    std::cout << "Введите фамилию: ";
    ExpInput expInput;
    human.lastName = expInput.isString(is);
    std::cout << "Введите имя: ";
    human.firstName = expInput.isString(is);
    std::cout << "Введите отчество: ";
    human.patronymic = expInput.isString(is);
    std::cout << "Введите год рождения: ";
    human.birthYear = expInput.isNumber(is, 1900, 2025);
    return is;
