...
// Класс автомобиля комфорт-класса, наследуется от TaxiVehicle
class ComfortCar : public TaxiVehicle {
private:
...
public:
    ComfortCar();                              // Конструктор по умолчанию
    ComfortCar(const std::string& brand, const std::string& model, int year,
               ...
               bool hasClimateControl, bool hasSeatHeating = false, 
    ...
    virtual ~ComfortCar() = default;           // Виртуальный деструктор
