...
// Класс минивэна, наследуется от TaxiVehicle
class Minivan : public TaxiVehicle {
private:
...
public:
    Minivan();                                 // Конструктор по умолчанию
    Minivan(const std::string& brand, const std::string& model, int year,
            ...
            bool hasChildSeats = true);         // Конструктор с параметрами
    virtual ~Minivan() = default;              // Виртуальный деструктор
