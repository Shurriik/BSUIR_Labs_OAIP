#ifndef BUSINESSCAR_H
#define BUSINESSCAR_H

#include "TaxiVehicle.h"

// Класс автомобиля бизнес-класса, наследуется от TaxiVehicle
class BusinessCar : public TaxiVehicle {
private:
...
public:
    BusinessCar();                             // Конструктор по умолчанию
    BusinessCar(const std::string& brand, const std::string& model, int year,
                ...
                bool hasLeatherSeats = true, bool hasPremiumAudio = true,
                bool hasChildSeats = false);   // Конструктор с параметрами
    virtual ~BusinessCar() = default;         // Виртуальный деструктор
